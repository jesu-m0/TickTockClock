...
interface IntervalCardProps {
      ...
      onUpdate: (updatedInterval: Interval) => void;
}
...
const IntervalCard = ({ interval, onDelete, onUpdate }: IntervalCardProps) => {
      ...
      const closeEditForm = () => {
            setOpenEditFormAnimation(false);
            setShowEditFormContent(false);
            setTimeout(() => setIsEditFormExpanded(false), 200);
            setTimeout(() => setDivEditFormExist(false), 900);
            setTimeout(() => setShowCardContent(true), 1000);
      }
                        ...
                        <div className="w-full flex flex-col lg:flex-row lg:justify-between gap-2">
                              <div className="flex gap-2 justify-start items-center">
                                    ...
                                    <div className={`flex items-center select-none transition-opacity duration-200 ${showCardContent ? 'opacity-100' : 'opacity-0'}`}>
                                          <div className="rounded-full w-10 h-10 border-2 border-surfaceDark dark:border-surface" style={{ backgroundColor: interval.color }} />
                                    </div>
                                          ...
                                          <button className={`p-2 hover:bg-charcoal rounded-lg transition-all group duration-200
                                                ...
                                                <svg className="w-8 h-8" viewBox="-2.4 -2.4 28.80 28.80" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                      <path
                                                            ...
                                                            className="fill-base dark:fill-muted group-hover:fill-surfaceDark dark:group-hover:fill-surface transition-colors"
                                                      />
                                                      <path
                                          ...
                                          <button onClick={handleDeleteClick} className={`p-2 rounded-lg transition-all duration-200 group ${showCardContent ? 'opacity-100' : 'opacity-0'}`}>
                                                <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                      <path
                                                            ...
                                                            className="fill-base dark:fill-muted group-hover:fill-secondary transition-colors"
                                                      />
                                                      <path
