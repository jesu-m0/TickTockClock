...
const IntervalCard = ({ interval, onDelete, onUpdate }: IntervalCardProps) => {
      ...
      return (
            <>
                  {/* Interval Card */}
                  <div id={interval.id.toString()} className="bg-timberwolf dark:bg-blackOlive rounded-xl p-2 flex gap-2">
                        {/* Drag Handle */}
                        ...
                        <div className={`flex items-center transition-opacity duration-200 ${showCardContent ? 'opacity-100' : 'opacity-0'}`}>
                              <p className="font-extrabold dark:font-bold dark:text-timberwolf text-blackOlive text-2xl">{interval.name}</p>
                        </div>

                        {/* Time */}
                        <div className={`flex items-center ml-auto bg-floralWhite dark:bg-timberwolf rounded-2xl px-4 py-2 transition-opacity duration-200 ${showCardContent ? 'opacity-100' : 'opacity-0'}`}>
                              <p className="text-blackOlive dark:text-eerieBlack text-2xl font-extrabold">
                  ...
                  {isDeleteModalOpen && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-lg">
                              <div className="bg-floralWhite dark:bg-blackOlive p-10 rounded-2xl shadow-lg max-w-md w-full">
                                    <h2 className="text-blackOlive dark:text-timberwolf text-5xl font-black mb-4">Are you sure?</h2>
