...
interface SoundOptionsGridProps {
  value: string;
  options: SoundOption[];
  ...
  /**
   * Tailwind classes for the chip's resting background.
   * Must contrast with the surrounding container's background to remain visible.
   */
  chipBgClass?: string;
}
...
const SoundOptionsGrid: React.FC<SoundOptionsGridProps> = ({
  value,
  options,
  funOptions,
  cheatMode,
  onChange,
  chipBgClass = 'bg-baseClr/10 dark:bg-baseClr',
}) => {
  ...
  const LoadingOverlay: React.FC<{ isHovered: boolean }> = ({ isHovered }) => (
    <>
      {isHovered && (
        <div
          className={`absolute inset-0 pointer-events-none ${cheatMode ? 'bg-gradient-to-r from-primary/40 via-secondary/40 to-tertiary/40' : 'bg-primary/25'}`}
          style={{ clipPath: `inset(0 ${(1 - loadingProgress) * 100}% 0 0)`, transition: 'clip-path 30ms linear' }}
        />
      )}
      {isHovered && loadingProgress >= 1 && (
        <div className={`absolute inset-0 rounded-full pointer-events-none ${cheatMode ? 'ring-2 ring-inset ring-secondary animate-pulse' : 'ring-2 ring-inset ring-primary/60'}`} />
      )}
    </>
  );
  ...
  return (
    <div className="flex flex-col gap-2">
      {cheatMode && (
        <div className="flex rounded-2xl overflow-hidden gap-1">
          ...
          return (
            <button
              ...
              className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-full
                ...
                ${isSelected ? 'bg-primary/25 ring-2 ring-primary' : `${chipBgClass} ring-1 ring-baseClr/10 dark:ring-muted/10`}
                ${isDimmed ? 'opacity-40 scale-95' : ''}
                ${isHovered && !cheatMode ? 'scale-105' : ''}`}
              style={isHovered && cheatMode ? getCheatAnimStyle(loadingProgress) : undefined}
            >
              <LoadingOverlay isHovered={isHovered} />
              <span className="text-sm z-10">{opt.icon}</span>
              <span className="text-xs lg:text-sm font-bold text-baseClr dark:text-muted z-10">{opt.label}</span>
            </button>
          );
        })}
      </div>
...
export default SoundOptionsGrid;
